...
import org.example.springboot231026.dto.dogsell.DogSellUploadResultDTO;
...
@RestController
public class DogSellUploadController {
    ...
    public ResponseEntity<List<DogSellUploadResultDTO>> uploadFile(MultipartFile [] multipartFiles){
        ...
        //반환타입 객체생성
        List<DogSellUploadResultDTO> dtoList= new ArrayList<>();
        ...
        for(MultipartFile multipartFile: multipartFiles){
            ...
            try {
                ...
                Thumbnails.of(savePath.toFile()).crop(Positions.CENTER).size(100,100)
                        .toFile(thumbFile);

                dtoList.add(new DogSellUploadResultDTO(originName, uuid, datePath));
